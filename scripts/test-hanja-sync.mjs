...
const appendix=JSON.parse(fs.readFileSync('data/hanja-memory/ebook-appendix.json','utf8'));
const appendixBefore=JSON.stringify(appendix);
assert.ok(!('comparisons' in book));
...
try {
  ...
  appendix.comparisons[0].cue='EBOOK-ONLY-COMPARE: a revised contrast.';
  fs.writeFileSync(path.join(temp,'ebook-appendix.json'),JSON.stringify(appendix));
  ...
  assert.ok(fs.readFileSync(path.join(temp,'content/31-comparisons.md'),'utf8').includes(appendix.comparisons[0].cue));
  assert.ok(!JSON.stringify(cardData).includes('EBOOK-ONLY-COMPARE'));
  const sharedBefore=fs.readFileSync(path.join(temp,'book.json'),'utf8');
  appendix.comparisons[0].cue='EBOOK-ONLY-SECOND-REVISION';
  fs.writeFileSync(path.join(temp,'ebook-appendix.json'),JSON.stringify(appendix));
  run();
  assert.equal(fs.readFileSync(path.join(temp,'book.json'),'utf8'),sharedBefore);
  assert.equal(fs.readFileSync(path.join(temp,'content/01-chapter.md'),'utf8'),chapter);
  ...
  assert.equal(JSON.stringify(JSON.parse(fs.readFileSync('data/hanja-memory/ebook-appendix.json','utf8'))),appendixBefore);
  console.log('PASS: shared Hanja content syncs; ebook appendix edits do not alter app data or lesson chapters.');
} finally {
